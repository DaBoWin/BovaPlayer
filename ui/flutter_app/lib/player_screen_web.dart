// ignore_for_file: deprecated_member_use, avoid_web_libraries_in_flutter, depend_on_referenced_packages
...
class WebPlayerScreen extends StatefulWidget {
  ...
  @override
  State<WebPlayerScreen> createState() => _PlayerScreenState();
}

class _PlayerScreenState extends State<WebPlayerScreen> {
  bool _isPlaying = false;
  final bool _hwAccelEnabled = true;
  String _currentFile = '';
  Timer? _frameTimer;
  int _frameWidth = 0;
  int _frameHeight = 0;
  ...
  Future<void> _initializeNativeBridge() async {
    try {
      ...
      debugPrint('Native bridge初始化结果: $result');
    } catch (e) {
      debugPrint('Native bridge初始化失败: $e');
    }
  }
  ...
  Future<void> _openFile() async {
    try {
      ...
      if (result != null) {
        ...
        debugPrint(
            '选择的文件: $fileName, 大小: ${(fileSize / 1024 / 1024).toStringAsFixed(1)} MB');
        ...
        if (kIsWeb) {
          try {
            ...
            if (isLargeFile) {
              ...
              debugPrint(
                  '处理大文件 (${(fileSize / 1024 / 1024).toStringAsFixed(1)} MB)，使用流式加载');
                  ...
                  SnackBar(
                    content: Row(
                      children: [
                        ...
                        Text(
                            '正在准备大文件流式播放 (${(fileSize / 1024 / 1024 / 1024).toStringAsFixed(1)} GB)...'),
                      ],
                    ),
              ...
              // 小文件：正常处理
              debugPrint(
                  '处理小文件 (${(fileSize / 1024 / 1024).toStringAsFixed(1)} MB)');
                  ...
                  SnackBar(
                    content: Row(
                      children: [
                        ...
                        const SizedBox(width: 16),
                        Text(
                            '正在加载视频文件 (${(fileSize / 1024 / 1024).toStringAsFixed(1)} MB)...'),
                      ],
                    ),
            ...
            // 显示初始化进度
            if (mounted && fileSize > 1024 * 1024 * 1024) {
              // 大于1GB
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Row(
        ...
        debugPrint('Native Bridge打开文件结果: $openResult');
      } else {
  ...
  Future<void> _togglePlay() async {
    if (_videoController != null && _videoController!.value.isInitialized) {
      ...
      if (_isPlaying) {
        ...
        debugPrint('Native Bridge暂停结果: $result');
        _stopFrameTimer();
      } else {
        final result = await NativeBridge.play();
        debugPrint('Native Bridge播放结果: $result');
        _startFrameTimer();
      }
      setState(() {
  ...
  Future<void> _stop() async {
    if (_videoController != null && _videoController!.value.isInitialized) {
      ...
      debugPrint('Native Bridge停止结果: $result');
      _stopFrameTimer();
      setState(() {
        _isPlaying = false;
        _currentPosition = 0;
  ...
  Future<void> _seekToPosition(double position) async {
    if (_videoController != null && _videoController!.value.isInitialized) {
      ...
      debugPrint('视频跳转到: ${position}s');
    } else {
      final result = await NativeBridge.seek(position);
      debugPrint('Native Bridge跳转结果: $result');
      setState(() {
