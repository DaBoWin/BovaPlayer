...
class _MediaKitPlayerPageState extends State<MediaKitPlayerPage> {
  ...
  static const _trafficLightsChannel =
      MethodChannel('com.bovaplayer/traffic_lights');
  ...
  void initState() {
    ...
    Future.microtask(() {
      if (!mounted) return;
      final isPro =
          Provider.of<AuthProvider>(context, listen: false).user?.isPro ??
              false;
      if (isPro) {
  ...
  void _initializeVideoController() async {
    ...
    print(
        '[MediaKitPlayer] 平台: ${Theme.of(context).platform}, 模拟器: $isEmulator, 硬件加速: $enableHwAccel');
    ...
    _speedTimer = Timer.periodic(
        const Duration(seconds: 1), (_) => _updateNetworkSpeed());
  ...
  Future<void> _testUrlAccess() async {
    try {
      ...
      print(
          '[MediaKitPlayer] Content-Length: ${response.headers.contentLength}');
  ...
  Future<void> _configureMpvTls(bool isEmulator) async {
    try {
      ...
      await (nativePlayer as dynamic)
          .setProperty('user-agent', 'BovaPlayer/1.0.0 (macOS; Flutter)');
      ...
      await (nativePlayer as dynamic)
          .setProperty('http-header-fields', headers.join('\r\n'));
      ...
      await (nativePlayer as dynamic)
          .setProperty('network-timeout', '120'); // 增加到120秒超时（大文件需要）
      await (nativePlayer as dynamic)
          .setProperty('http-reconnect', 'yes'); // 启用自动重连
      await (nativePlayer as dynamic)
          .setProperty('stream-open-timeout', '120'); // 流打开超时120秒
      ...
      await (nativePlayer as dynamic)
          .setProperty('stream-lavf-o-append', 'tcp_keepalive=1');

      // 多线程解码配置
      await (nativePlayer as dynamic)
          .setProperty('vd-lavc-threads', '4'); // 视频解码线程数
      await (nativePlayer as dynamic)
          .setProperty('ad-lavc-threads', '2'); // 音频解码线程数
      await (nativePlayer as dynamic)
          .setProperty('demuxer-thread', 'yes'); // 启用解复用线程

      // 音频解码器回退策略 - 关键：TrueHD 不支持时自动切换音轨
      await (nativePlayer as dynamic)
          .setProperty('audio-fallback-to-null', 'yes'); // 音频失败时不中断播放
      await (nativePlayer as dynamic)
          .setProperty('ad-lavc-downmix', 'yes'); // 自动降混音
      await (nativePlayer as dynamic)
          .setProperty('audio-channels', 'stereo'); // 强制立体声输出
      await (nativePlayer as dynamic)
          .setProperty('alang', 'jpn,eng,chi,zh,zho'); // 音轨语言优先级

      // 缓冲配置 - 针对杜比视界等超高码率视频优化
      await (nativePlayer as dynamic)
          .setProperty('demuxer-max-bytes', '150000000'); // 150MB（杜比视界需要更大缓冲）
      await (nativePlayer as dynamic)
          .setProperty('demuxer-max-back-bytes', '75000000'); // 75MB
      await (nativePlayer as dynamic)
          .setProperty('demuxer-readahead-secs', '10'); // 预读10秒（增加稳定性）

      // 快速启动播放 - 针对高码率优化
      await (nativePlayer as dynamic)
          .setProperty('cache-pause-initial', 'yes'); // 等待初始缓存
      await (nativePlayer as dynamic)
          .setProperty('cache-pause-wait', '5'); // 等待5秒缓存（高码率需要更多时间）
      await (nativePlayer as dynamic)
          .setProperty('cache-secs', '20'); // 缓存20秒（杜比视界需要更多缓冲）

      // 预读优化 - 启用快速预读
      await (nativePlayer as dynamic)
          .setProperty('cache-on-disk', 'no'); // 内存缓存更快
      await (nativePlayer as dynamic)
          .setProperty('demuxer-donate-buffer', 'yes'); // 捐赠缓冲区给解码器

      // HTTP 范围请求优化（支持多线程下载）
      await (nativePlayer as dynamic)
          .setProperty('stream-buffer-size', '8192'); // 8KB 流缓冲（增加吞吐量）
      ...
      await (nativePlayer as dynamic)
          .setProperty('demuxer-lavf-analyzeduration', '5'); // 5秒分析时间（杜比视界需要更多）
      await (nativePlayer as dynamic)
          .setProperty('demuxer-lavf-probesize', '50000000'); // 50MB 探测大小（超高码率）
      await (nativePlayer as dynamic)
          .setProperty('demuxer-lavf-probe-info', 'auto'); // 自动探测

      // 启用 MPV 详细日志以查看多线程状态
      await (nativePlayer as dynamic)
          .setProperty('msg-level', 'all=info,ffmpeg=debug');
          ...
          children: [
            ...
            IconButton(
              icon: Icon(
                ...
                color:
                    _showStatsPanel ? Colors.deepPurpleAccent : Colors.white70,
                size: 20,
              ),
              onPressed: () {
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      _buildToolIconButton(Icons.fast_rewind_rounded,
                      ...
                      const SizedBox(width: 16),
                      _buildToolIconButton(Icons.fast_forward_rounded,
                          () => _seek(const Duration(seconds: 10))),
                    ],
                  ),
        ...
        return Row(
          children: [
            Text(
              _isDraggingProgress
                  ? _formatDuration(
                      Duration(milliseconds: _dragProgressPosition.toInt()))
                  ...
                  return GestureDetector(
                    ...
                    onHorizontalDragEnd: (details) {
                      _player.seek(Duration(
                          milliseconds: _dragProgressPosition.toInt()));
                      setState(() {
          ...
          children: [
            ...[0.5, 0.75, 1.0, 1.25, 1.5, 2.0].map((speed) {
              return _buildActionItem(
                  '${speed}x', true, _playbackSpeed == speed, () {
                _changeSpeed(speed);
                Navigator.pop(context);
              });
            }),
        ...
        Column(
          ...
          children: [
            ...[
              ...
              return _buildActionItem(
                  ratio['label']!, true, _aspectRatio == ratio['value'], () {
  ...
  Widget _buildStatsPanel() {
    ...
    final stats = <String, String>{
      ...
      '音频码率': state.audioBitrate != null
          ? '${(state.audioBitrate! / 1000).toStringAsFixed(0)} Kbps'
          : '未知',
